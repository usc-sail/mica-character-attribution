...
def preprocess_logits_for_metrics(logits: torch.Tensor, _) -> torch.Tensor:
    """Preprocess logits to prepare for metrics computation for the supervized fine-tuning method.
...
def train(partial_state: PartialState,
    ...
    if partial_state.is_local_main_process:
        logging.info("\ntokens/sample:")
        logging.info(f"{train_and_dev_dataset_name} train: max = {max(train_ntokens)}, min = {min(train_ntokens)}, "
                     f"95%tile = {np.quantile(train_ntokens, 0.95):.1f}")
        logging.info(f"{train_and_dev_dataset_name} dev: max = {max(dev_ntokens)}, min = {min(dev_ntokens)}, "
    ...
    for dataset, df, name, partition in zip(datasets, dfs, dataset_names, partitions):
        ...
        log(f"{metrics}\n\n")
        if partial_state.is_local_main_process:
            predictions_file = os.path.join(experiments_dir, f"{name}-{partition}.csv")
            metrics_file = os.path.join(experiments_dir, f"{name}-{partition}.json")
...
def predict(partial_state: PartialState, datasetname_to_data: Dict[str, List]):
    ...
    for dataset_name, datalist in datasetname_to_data.items():
        matched_dataset_name = re.match("(chatter-contexts)|(chatter-segments)|(personet)", dataset_name).group(0)
        size = int(dataset_name.split("-")[4]) if matched_dataset_name == "chatter-contexts" else 2000
        dataset, df = data.create_sft_dataset(data=datalist,
                                              tokenizer=tokenizer,
