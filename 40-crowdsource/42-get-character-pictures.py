"""Get character pictures from imdb"""
import os
import re
import tqdm
import random
...
flags.DEFINE_string("input_file", default="40-crowdsource/input.csv",
                    help="input csv file containing fields for character and imdb character urls")
flags.DEFINE_string("output_file", default="40-crowdsource/input-with-pics.csv",
...
def crawl_imdb_pictures(_):
    ...
    for cid, df in tqdm.tqdm(input_df.groupby("character"), desc="crawl character pictures", unit="character",
                             total=input_df["character"].unique().size):
    ...
    output_df = pd.DataFrame(rows, columns=["character", "picture-1", "picture-2", "picture-3"])
    join_df = input_df.merge(output_df, on="character", how="left")
