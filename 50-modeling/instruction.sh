...
CMD="accelerate launch --config_file deepspeed.yaml 509-finetune.py --instrtune --train_dataset chatter \
...
for SIZE in 2000,3500 1000,1800 1500,2700 500,1000 250,550; do
    set -- $SIZE
    $CMD --contexts_file 25P-${1}C-all-mpnet-base-v2-0.05NEG-0.05POS.jsonl --max_seq_len $2
    for STRATEGY in first last random; do
        $CMD --contexts_file 25P-${1}C-${STRATEGY}.jsonl --max_seq_len $2
    done
done
IFS=$OLDIFS