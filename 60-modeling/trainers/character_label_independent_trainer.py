"""Train and evaluate the label independent model on character segments"""
import os
import math
import tqdm
import torch
    ...
    for epoch in range(n_epochs):
        ...
        for batch in tbar:
            ...
            for character in batch_characters:
                ...
                # concatenate the character embeddings from each movie, shape = nmovies x hidden-size
                character_embeddings = torch.cat(character_embeddings_arr, dim=0)

                # encode character tropes, shape = nlabels x hidden-size
                trope_embeddings = model.encoder(character_trope_token_ids).pooler_output

                # reshape character embeddings and trope embeddings for classification
                # final shape will be nlabels x nmovies x hidden-size
                nmovies = len(character_embeddings)
                nlabels = len(trope_embeddings)
                character_embeddings = character_embeddings.unsqueeze(dim=0).expand(nlabels, -1, -1)
                trope_embeddings = trope_embeddings.unsqueeze(dim=1).expand(-1, nmovies, -1)

                # concatenate embeddings to get shape nlabels x nmovies x 2 hidden-size
                character_trope_embeddings = torch.cat([character_embeddings, trope_embeddings], dim=2)

                # classify whether character portrays trope
                logits = classifier(character_trope_embeddings)
