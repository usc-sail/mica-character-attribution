...
flags.DEFINE_integer("longformerattn", default=512, help="sliding attention window of longformer model")
flags.DEFINE_bool("lbl", default=False, help="use label-dependent model")
...
# training params
flags.DEFINE_integer("ep", default=10, help="epochs")
...
flags.DEFINE_integer("tokbatch", default=80, help="tokens in '000s per batch")
...
def start_training(_):
    ...
    for flagobj in FLAGS.flags_by_module_dict()["60-modeling/64-train.py"]:
        logging.info(f"{flagobj.help:80s} = {flagobj.value}")
    logging.info("")
